		...
		if ($action != 'presend') {
			...
			if (empty($reshook)) {
				...
				if (isModEnabled('subtotals') && getDolGlobalString('SUBTOTAL_TITLE_'.strtoupper($object->element))) {
					...
					$url_button[] = array(
						'lang' => 'subtotals',
						'enabled' => (isModEnabled('supplier_invoice') && $object->status == FactureFournisseur::STATUS_DRAFT),
						'perm' => (bool) $usercancreate,
						'label' => $langs->trans('AddTitleLine'),
						'url' => '/fourn/facture/card.php?id='.$object->id.'&action=add_title_line&token='.newToken()
					);
					...
					$url_button[] = array(
						...
						'url' => '/fourn/facture/card.php?id='.$object->id.'&action=add_subtotal_line&token='.newToken()
					);
