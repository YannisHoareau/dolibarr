...
if (empty($reshook)) {
	...
	if ($action == 'addline' && $permissiontoadd) {
		...
		foreach (FactureFournisseurRec::$TITLE_OPTIONS as $option) {
		...
		foreach (FactureFournisseurRec::$SUBTOTAL_OPTIONS as $option) {
			$value = GETPOST($option, 'alphanohtml');
			if ($value) {
				$subtotal_options[$option] = $value == 'on' ? 1 : $value;
			}
...
if ($action == 'create') {
	...
	if ($object->id > 0) {
		...
		if (empty($object->suspended) && isModEnabled('subtotals') && getDolGlobalString('SUBTOTAL_TITLE_'.strtoupper($object->element))) {
			...
			$url_button[] = array(
				'lang' => 'subtotals',
				'enabled' => (isModEnabled('invoice') && $object->status == FactureFournisseurRec::STATUS_DRAFT),
				'perm' => (bool) $usercancreate,
				'label' => $langs->trans('AddTitleLine'),
				'url' => '/fourn/facture/card-rec.php?facid='.$object->id.'&action=add_title_line&token='.newToken()
			);
			...
			$url_button[] = array(
				...
				'url' => '/fourn/facture/card-rec.php?facid='.$object->id.'&action=add_subtotal_line&token='.newToken()
			);
