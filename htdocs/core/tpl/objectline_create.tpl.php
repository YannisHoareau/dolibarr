...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	<td class="nobottom linecoldescription minwidth400imp">
		...
		if (isModEnabled('subtotal')) { ?>
			<div class="subtotal_options">
				<!-- Bouton radio pour ligne de titre -->
				<div>
					<input type="radio" class="subtotal" name="prod_entry_mode" id="subtotal_title" value="title">
					<label for="subtotal_title" id="title_inputs">
						<span class="textradioforitem" disabled=""><?php echo $langs->trans("SubtotalTITLE"); ?></span>
						<input type="text" name="dp_desc" id="title_name" placeholder="<?php echo $langs->trans("Name"); ?>" disabled="">
						<select name="qty" id="title_level" disabled="">
							<option value="1">1</option>
							<option value="6514">6514</option>
						</select>
						<input type="hidden" name="type" id="title_type" value="9">
						<input type="hidden" name="special_code" id="title_special_code" value="811">
					</label>
				</div>

				<!-- Bouton radio pour ligne de sous-total -->
				<div>
					...
					<label for="subtotal_st" id="subtotal_inputs">
						<span class="textradioforitem" disabled=""><?php echo $langs->trans("SubtotalST"); ?></span>
						<input type="text" name="dp_desc" id="subtotal_name" placeholder="<?php echo $langs->trans("Name"); ?>" disabled="">
						<select name="qty" id="subtotal_level" disabled="">
							<option value="1">1</option>
							<option value="6514">6514</option>
						</select>
						<input type="hidden" name="type" id="subtotal_type" value="9">
						<input type="hidden" name="special_code" id="subtotal_special_code" value="812">
					</label>
				</div>
			</div>
			...
			<script>
				document.addEventListener("DOMContentLoaded", function () {
					// Obtenir les boutons radio et champs à cacher/montrer
					const radioTitle = document.getElementById("subtotal_title");
					const radioSubtotal = document.getElementById("subtotal_st");
					const inputsTitle = document.getElementById("title_inputs");
					const inputsSubtotal = document.getElementById("subtotal_inputs");
					const radioFreeLine = document.getElementById("prod_entry_mode_free");
					const radioProductLine = document.getElementById("prod_entry_mode_predef");
					const columns = document.querySelectorAll("#tva_tx, #price_ht, #price_ttc, #qty, #remise_percent");
					const fieldsToToggle = [];
					const titleInputsToToggle = [];
					const subtotalInputsToToggle = [];

					columns.forEach(field => {
						const nodes = field.parentNode.childNodes;
						for (let i = 0; i < nodes.length; i++) {
							if (typeof nodes[i].style !== "undefined") {
								fieldsToToggle.push(field.parentNode.childNodes[i]);
							}
						}
						fieldsToToggle.push(document.querySelector("#trlinefordates"))
						fieldsToToggle.push(document.querySelector("#dp_desc"))
					});

					inputsTitle.childNodes.forEach(field => {
						if (typeof field.style !== "undefined") {
							titleInputsToToggle.push(field);
						}
					});

					inputsSubtotal.childNodes.forEach(field => {
						if (typeof field.style !== "undefined") {
							subtotalInputsToToggle.push(field);
						}
					});

					console.log(titleInputsToToggle, subtotalInputsToToggle)

					function toggleFields() {
						if (radioTitle.checked || radioSubtotal.checked) {
							fieldsToToggle.forEach(field => {
								field.disabled = true;
								field.style.display = "none";
							});
							if (radioSubtotal.checked) {
								titleInputsToToggle.forEach(field => {
									field.disabled = true;
								});
								subtotalInputsToToggle.forEach(field => {
					...
					// Ajout des listeners
					radioTitle.addEventListener("change", toggleFields);
					radioSubtotal.addEventListener("change", toggleFields);
					radioFreeLine.addEventListener("change", toggleFields);
					radioProductLine.addEventListener("change", toggleFields);
